...
##Layer=vector
##Criterion=selection cp;bic;r2;adjr2;tous
...
X<-X[,-a]
b<-which(colnames(X)==ID_field)
X<-X[,-b]
Y<-Layer[[Field1]]
methods=c("exhaustive", "backward", "forward", "seqrep")
meth<-methods[Method + 1]
test1<-regsubsets(Y~X,data=Layer, really.big=T,nbest=1,nvmax=nbr_var, intercept=F,method=meth)
